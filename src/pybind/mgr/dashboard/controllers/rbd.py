...
class Rbd(RESTController):
    ...
    def move_trash(self, pool_name, image_name, delay=0):
        def _move_trash(ioctx):
            rbd_inst = rbd.RBD()
            rbd_inst.trash_move(ioctx, image_name, delay)

        return _rbd_call(pool_name, _move_trash)
...
class RbdTrash(RESTController):
    ...
    def _rbd_pool_list(self, pool_name):
        rbd_inst = rbd.RBD()
        with mgr.rados.open_ioctx(pool_name) as ioctx:
            images = rbd_inst.trash_list(ioctx)
            result = []
            for trash in images:
            ...
            for image in pool['value']:
                if image['deferment_end_time'] < now:
                    def _remove_trash(ioctx):
                        ...
                        rbd_inst.trash_remove(ioctx, image['id'], 0)

                    _rbd_call(pool['pool_name'], _remove_trash)

    # Restore an image from trash.
    @RbdTask('trash/restore', ['{pool_name}', '{image_id}', '{new_image_name}'], 2.0)
    @RESTController.Resource('POST')
    def restore(self, pool_name, image_id, new_image_name):
        def _restore_trash(ioctx):
            rbd_inst = rbd.RBD()
            rbd_inst.trash_restore(ioctx, image_id, new_image_name)
    ...
    def delete(self, pool_name, image_id, image_name, force=False):
        def _remove_trash(ioctx):
            rbd_inst = rbd.RBD()
            rbd_inst.trash_remove(ioctx, image_id, int(str_to_bool(force)))

        return _rbd_call(pool_name, _remove_trash)
