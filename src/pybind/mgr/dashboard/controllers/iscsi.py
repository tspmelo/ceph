        ...
        try:
            ...
            for client in clients:
                ...
                for lun in client['luns']:
                    ...
                    for group in groups:
                        if client_iqn in group['members']:
                            group_disks = list(map(lambda x: '{}/{}'.format(x['pool'], x['image']), group['disks']))
                    if not target_config or client_iqn not in target_config['clients'] or \
                            (image_id not in target_config['clients'][client_iqn]['luns'] and \
                            image_id not in group_disks):
