...
class IscsiClient(RestClient):
    ...
    def instance(cls, gateway_name=None):
        ...
        settings = (service_url,
                    Settings.ISCSI_API_USERNAME,
                    Settings.ISCSI_API_PASSWORD)

        instance = cls._instances.get(gateway_name)
        if not instance or settings != instance.settings:
            ...
            auth = HTTPBasicAuth(Settings.ISCSI_API_USERNAME, Settings.ISCSI_API_PASSWORD)
            instance = IscsiClient(host, port, IscsiClient._CLIENT_NAME,
                                   ssl, auth)
            instance.settings = (
                service_url,
                Settings.ISCSI_API_USERNAME,
